...
export class IncomeExpenses {
    ...
    constructor(openNewRoute: (url: string) => Promise<void>) {
        ...
        this.getAllOperations(PeriodForButtonEnum.day).then();
    }
    ...
    private async getAllOperations(period: string): Promise<void> {
    ...
    async getAllOperationsInterval(period: string, body: OperationsResponseBodyType): Promise<void> {
    ...
    private async createTableIncomeExpenses(operations: OperationType[]): Promise<void> {
        ...
        for (let i: number = 0; i < operations.length; i++) {
            ...
            if (operations[i].type === 'income') {
                let trElementColor: HTMLTableCellElement = trElement.insertCell();
